...
//   const { courseId, userId, username } = useParams(); // Get courseId and userId from URL
  ...
  useEffect(() => {
    ...
    axios
      .get(`http://localhost:5000/api/discussion/${courseId}/comments`)
      .then((response) => {
  ...
  const handleCommentSubmit = () => {
    ...
    axios
      .post(`http://localhost:5000/api/discussion/${courseId}/comments`, commentData)
      .then((response) => {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="bg-white shadow-md rounded-lg p-6">
        <h1 className="text-2xl font-semibold text-gray-800 mb-4">Course Discussion</h1>
          ...
          return (
            <div
              ...
              className={`p-4 mb-4 rounded-lg flex items-center space-x-4 ${
                isUserComment ? "bg-blue-100 border border-blue-400" : "bg-gray-100"
              }`}
            >
              ...
              {/* Comment Content */}
              <div>
                <p className={`font-bold ${isUserComment ? "text-blue-700" : "text-gray-800"}`}>
                  {comment.username}
                </p>
